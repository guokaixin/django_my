...
def datetime_to_localtime(date_time, return_format='%Y-%m-%d %H:%M:%S'):
    """
    返回本地时间，格式为datetime或者datetime_str
    :param date_time: datetime类型的值
    :param return_format: 返回时间字符串的格式
    :return:
    """
    if date_time.tzinfo == pytz.utc:
        local_time = timezone.localtime(date_time)
    else:
        local_time = timezone.make_aware(date_time)
    if return_format:
        if local_time:
            return local_time.strftime(return_format)
        return ''
    return local_time
